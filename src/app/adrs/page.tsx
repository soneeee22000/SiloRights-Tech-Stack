...
import {
  FileCode, CheckCircle, AlertTriangle, Info,
  ChevronDown, ChevronRight, Server, Cloud,
  Lock, Zap, Database, Globe
} from 'lucide-react';
...
const adrs: ADR[] = [
  {
    ...
    status: 'Accepted',
    context: 'Building a platform with distinct domains (Catalog, Licensing, Compliance, Payments) that need to scale independently.',
    ...
    mitigation: 'Start with "modular monolith" that can be split later. Services share database initially, separate post-MVP.',
    icon: Server,
    color: 'blue'
  },
  {
    ...
    status: 'Accepted',
    context: 'Services need to communicate without tight coupling. License events must trigger blockchain recording and notifications.',
    decision: 'Use Redis Pub/Sub for internal events, Celery for async job processing.',
    consequences: {
      positive: [
    ...
    mitigation: 'Implement saga pattern for multi-step transactions. Use correlation IDs for tracing.',
    icon: Zap,
    color: 'purple'
  },
  {
    ...
    status: 'Accepted',
    context: 'EU AI Act compliance requires demonstrable EU data residency. Customer data must not leave EU jurisdiction.',
    decision: 'Use Scaleway (France) for all infrastructure. Avoid AWS/GCP/Azure.',
    consequences: {
      positive: [
    ...
    mitigation: 'Use standard tools (Kubernetes, PostgreSQL) that work on any cloud. Plan for multi-cloud if needed.',
    icon: Cloud,
    color: 'amber'
  },
  {
    ...
    status: 'Accepted',
    context: 'EU AI Act requires auditable proof of licensing. Immutability is crucial for legal disputes.',
    ...
    mitigation: 'Store only hashes on-chain. Full data off-chain in PostgreSQL. Use gas price alerts.',
    icon: Lock,
    color: 'indigo'
  },
  {
    ...
    status: 'Accepted',
    context: 'Need ACID compliance, complex queries, JSONB for flexible metadata, and EU-hostable solution.',
    decision: 'Use PostgreSQL 16 as the primary database with read replicas for scaling.',
    consequences: {
      positive: [
    ...
    mitigation: 'Use read replicas for read-heavy workloads. Consider Citus for horizontal scaling if needed.',
    icon: Database,
    color: 'orange'
  },
  {
    ...
    status: 'Accepted',
    context: 'Need fast, typo-tolerant search for catalog discovery. Must be EU-hostable.',
    ...
    mitigation: 'Sufficient for our use case. Can migrate to Elasticsearch later if needed.',
    icon: Globe,
    color: 'emerald'
  }
];
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-slate-400">Key technical decisions and their rationale</p>
            </div>
          </div>
          <p className="text-slate-400 max-w-3xl mt-4">
            ADRs document significant architectural decisions made during the design of SILOETT Rights.
            Each record captures the context, decision, and consequences to help future developers
        ...
        <div className="grid grid-cols-3 gap-6 mb-12">
          <div className="bg-gradient-to-br from-emerald-500/10 to-transparent rounded-xl p-6 border border-emerald-500/20">
            <div className="text-3xl font-bold text-emerald-400 mb-1">{adrs.filter(a => a.status === 'Accepted').length}</div>
          ...
          <div className="bg-gradient-to-br from-amber-500/10 to-transparent rounded-xl p-6 border border-amber-500/20">
            <div className="text-3xl font-bold text-amber-400 mb-1">{adrs.filter(a => a.status === 'Proposed').length}</div>
          ...
          <div className="bg-gradient-to-br from-slate-500/10 to-transparent rounded-xl p-6 border border-slate-500/20">
            <div className="text-3xl font-bold text-slate-400 mb-1">{adrs.filter(a => a.status === 'Deprecated').length}</div>
          ...
          {adrs.map((adr) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br from-${adr.color}-500 to-${adr.color}-600 flex items-center justify-center`}>
                  ...
                  <div className="flex-1 text-left">
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded border ${getStatusColor(adr.status)}`}>
                        {adr.status}
                      </span>
                    </div>
                ...
                {isExpanded && (
                  <div className="px-6 pb-6 border-t border-white/10">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                      ...
                      <div className="space-y-4">
                        <div>
                          <h4 className="text-sm font-semibold text-slate-300 mb-2">Consequences</h4>
                          <div className="space-y-3">
                            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-4">
                              <div className="text-xs font-semibold text-emerald-400 mb-2">Positive</div>
                            ...
                            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4">
                              <div className="text-xs font-semibold text-amber-400 mb-2">Trade-offs</div>
                              <ul className="space-y-1">
                        ...
                        {adr.mitigation && (
                          <div>
                            <h4 className="text-sm font-semibold text-slate-300 mb-2">Mitigation Strategy</h4>
            ...
            {[
              {
                ...
                usage: 'License request → approval → payment → blockchain recording'
              },
              {
                name: 'Event Sourcing',
                desc: 'Immutable audit trail for all license operations',
                usage: 'Every state change stored as event for replay and audit'
              },
              {
                name: 'CQRS',
                desc: 'Separate read/write models for catalog operations',
                usage: 'Optimized search queries independent of write operations'
              }
            ].map((pattern, i) => (
